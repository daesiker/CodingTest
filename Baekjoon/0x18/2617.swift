...
var lightGraph = Array(repeating: [Int](), count: n + 1)
var heavyGraph = Array(repeating: [Int](), count: n + 1)

for _ in 0..<m {
    let tmp = readLine()!.split(separator: " ").map { Int($0)! }
    lightGraph[tmp[0]].append(tmp[1])
    heavyGraph[tmp[1]].append(tmp[0])
}
...
for i in 1...n {
    let x = bfs(lightGraph, i)
    let y = bfs(heavyGraph, i)
    if x >= mid || y >= mid {
        answer += 1
    }
}
...
func bfs(_ graph: [[Int]], _ node: Int) -> Int {
