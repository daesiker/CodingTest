...
// 번째 줄에는 개강총회를 시작한 시간 S, 개강총회를 끝낸 시간 E, 개강총회 스트리밍을 끝낸 시간 Q가 주어진다. (00:00 ≤ S < E < Q ≤ 23:59)
// 각 시간은 HH:MM의 형식으로 주어진다.

// 두번째 줄부터는 HI-ARC에서 방송하는 스트리밍 영상의 채팅 기록들이 시간순으로 주어지는데, 
// (시간) (학회원 닉네임)의 형태로 주어진다. 학회원의 닉네임은 알파벳 대소문자와 숫자, 그리고 특수 기호(., _, -)로만 구성된 문자열이며 최대 20글자이다.
...
let (s, e, q) = (hToMin(seq[0]), hToMin(seq[1]), hToMin(seq[2]))
...
func hToMin(_ h: String) -> Int {
    let h = h.split(separator: ":").map { Int($0)! }
    return h[0] * 60 + h[1]
}